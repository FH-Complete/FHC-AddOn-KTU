...
echo "<option disabled ".(isset($prestudent_id) ? "" : "selected")." value>".$p->t('global/auswaehlen')."</option>";

foreach ($prestudent->result as $pres)
{
	$studiengang = new studiengang($pres->studiengang_kz);
	echo "<OPTION ".(isset($prestudent_id) && $prestudent_id == $pres->prestudent_id ? "selected" : "")." value='setVerlaengerung.php?prestudent_id=$pres->prestudent_id'>$studiengang->bezeichnung</OPTION>";
}
echo "</SELECT>";
...
if(isset($prestudent_id))
{
	...
	echo "<input type='hidden' value='".$prestudent_id."' name='prestudent_id'>";
}
